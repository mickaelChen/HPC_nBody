...
void bodies_Compute_own_interaction(bodies_t *FMB_RESTRICT p_b){
  ...
  #pragma omp parallel for schedule(runtime) private(j,pix,piy,piz,val_i,pjx,pjy,pjz,val_j)
  for(i=0; i<n-1; i++){
    pix = p_px[i];
    piy = p_py[i];
    piz = p_pz[i];
...
#pragma omp parallel for schedule(static) private(j,k,vpix,vpiy,vpiz,vvali,vfix,vfiy,vfiz,vpjx,vpjy,vpjz,vvalj, pj_loc_fx, pj_loc_fy, pj_loc_fz)
  for(i=0; i<n; i+=8)
    {
      ...
      for (j=0; j<n; j++)
        {
          ...
          for (k = 0; k < 8; ++k)
            {
              CHECK_IF_POSITION_ARE_TOO_CLOSE(p_px[i+k], p_py[i+k], p_pz[i+k], pj_pos_x[j], pj_pos_y[j], pj_pos_z[\
														  j]);
            }
					   ...
					   COORDINATES_T *pj_fx, COORDINATES_T *pj_fy, COORDINATES_T *pj_fz, VALUES_T *pj_values, int h)
{
  ...
  #pragma omp parallel for schedule(runtime) private(j,pix,piy,piz,val_i,fix,fiy,fiz,pjx,pjy,pjz,val_j)
  for(i=0; i<n; i++){
    pix = p_px[i];
    piy = p_py[i];
    piz = p_pz[i];
